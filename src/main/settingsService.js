...
function normalizeSettings(input = {}) {
  ...
  return {
    ...
    company: {
      name: String(merged.company?.name || '').trim() || 'GPA 1.0.0',
      logo_path: merged.company?.logo_path || null,
      logo_file_name: merged.company?.logo_file_name || null,
      document_header: String(merged.company?.document_header || '').trim() || String(merged.company?.name || '').trim() || 'GPA 1.0.0',
